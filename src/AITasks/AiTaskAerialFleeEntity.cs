...
{
    public class AiTaskAerialFleeEntity : AiTaskBase
    {
        ...
        public AiTaskAerialFleeEntity(EntityAgent entity) : base(entity)
        {
        }
        ...
        private void updateTargetPos()
        {
            ...
            double randY = 0;
            bool avoidFall = false;

            if (entity.Properties.Habitat == EnumHabitat.Air)
            {
                randY = entity.ServerPos.Y >= targetEntity.ServerPos.Y ? 5 * GameMath.DEG2RAD * 50 * 0.02f : -5 * GameMath.DEG2RAD * 50 * 0.02f;
            }

            // Some simple steering behavior, works really suxy
            tmpVec = tmpVec.Set(entity.ServerPos.X, entity.ServerPos.Y, entity.ServerPos.Z);
            avoidFall = watchCliffs(tmpVec.AheadCopy(10, 0, yaw - GameMath.PI / 2).AsBlockPos);
            tmpVec.Ahead(0.9, 0, yaw - GameMath.PI / 2);
            // Running into wall?
            if (traversable(tmpVec) && avoidFall)
            {
                targetPos.Set(entity.ServerPos.X, entity.ServerPos.Y, entity.ServerPos.Z).Ahead(10, randY, yaw - GameMath.PI / 2);
                return;
            }

            // Try 90 degrees left
            tmpVec = tmpVec.Set(entity.ServerPos.X, entity.ServerPos.Y, entity.ServerPos.Z);
            avoidFall = watchCliffs(tmpVec.AheadCopy(10, 0, yaw - GameMath.PI).AsBlockPos);
            tmpVec.Ahead(0.9, 0, yaw - GameMath.PI);
            if (traversable(tmpVec) && avoidFall)
            {
                targetPos.Set(entity.ServerPos.X, entity.ServerPos.Y, entity.ServerPos.Z).Ahead(10, randY, yaw - GameMath.PI);
                return;
            }

            // Try 90 degrees right
            tmpVec = tmpVec.Set(entity.ServerPos.X, entity.ServerPos.Y, entity.ServerPos.Z);
            avoidFall = watchCliffs(tmpVec.AheadCopy(10, 0, yaw).AsBlockPos);
            tmpVec.Ahead(0.9, 0, yaw);
            if (traversable(tmpVec) && avoidFall)
            {
                targetPos.Set(entity.ServerPos.X, entity.ServerPos.Y, entity.ServerPos.Z).Ahead(10, randY, yaw);
                return;
            }
        ...
        bool watchCliffs(BlockPos pos)
        {
            if (entity.Properties.Habitat != EnumHabitat.Land) return true;

            for (int i = 1; i < 6; i++)
            {
                Block ground = entity.World.BlockAccessor.GetBlock(pos.Down(i));
                if (ground.SideSolid[BlockFacing.UP.Index]) return true;
            }
