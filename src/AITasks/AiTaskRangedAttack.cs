...
namespace CaelumTerras
{
  ...
  projectiles = An array of potential projectiles to shoot.A projectile should look like
    ...
    public class AiTaskRangedAttack : AiTaskBase, IWorldIntersectionSupplier
    {
        Entity targetEntity;
        ...
        int burstCooldownMs = 100;
        int burstSize = 1;
        float spreadAngle = 0;
        int projectileCount = 1;
        float shotVelocity = 1;
        AmmuntionPouch[] ammo = new AmmuntionPouch[0];
        ...
        public AiTaskRangedAttack(EntityAgent entity) : base(entity)
        {
        }
        ...
        public override void LoadConfig(JsonObject taskConfig, JsonObject aiConfig)
        {
            ...
            this.spreadAngle = taskConfig["spreadAngle"].AsFloat(1f);
            ...
            if (taskConfig["projectiles"] != null)
            {
                JsonObject[] blueprints = taskConfig["projectiles"].AsArray();

                List<AmmuntionPouch> pouches = new List<AmmuntionPouch>();

                foreach (JsonObject blueprint in blueprints)
                {
                    JsonItemStack stack = blueprint["stack"].AsObject<JsonItemStack>();
                    if (stack != null) stack.Resolve(entity.World, "Ranged Attack");

                    AmmuntionPouch pouch = new AmmuntionPouch()
                    {
                        Projectile = new AssetLocation(blueprint["projectileEntityCode"].AsString("arrow-copper")),
                        Damage = blueprint["damage"].AsFloat(),
                        Weight = blueprint["weight"].AsFloat(0.1f),
                        DropChance = blueprint["dropChance"].AsFloat(0f),
                        ProjectileStack = stack?.ResolvedItemstack
                    };

                    pouches.Add(pouch);
                }

                this.ammo = pouches.ToArray();
            }
        ...
        public override bool ContinueExecute(float dt)
        {
            ...
            if (shotsFired < burstSize && nextShotMs < entity.World.ElapsedMilliseconds)
            {
                //System.Diagnostics.Debug.WriteLine("Fired! x" + (shotsFired + 1));
                if (hasDirectContact(targetEntity))
                {
                    for (int i = 0; i < projectileCount; i++)
                    {
                        AmmuntionPouch pouch = ammo[entity.World.Rand.Next(ammo.Length)];
                        ...
                        projectile = pouch.getRangedEntity(projectile, entity, entity.Stats.GetBlended("rangedWeaponsDamage"));

                        

                        Vec3d pos = entity.ServerPos.XYZ.Add(0, entity.LocalEyePos.Y - 0.2, 0);
                        Vec3d aheadPos = pos.AheadCopy(1, pitch + randomizeAngle(spreadAngle), entity.ServerPos.Yaw + (93 * GameMath.DEG2RAD * 50 * 0.02f) + randomizeAngle(spreadAngle));
                ...
                shotsFired += 1;
                if (shotsFired < burstSize) nextShotMs += burstCooldownMs + (int)(burstCooldownMs * (1 - entity.Stats.GetBlended("rangedWeaponsSpeed")));
            }
